...
class AccountingRepository(
    ...
    // Phase 7J audit, Section 2 (duplicate-posting risk) - confirmed defect: postVoucher's
    // idempotencyKey/voucherNumber are both freshly generated by most callers on every invocation
    // (UI-level `isSubmitting` flags only guard a single composable instance's lifetime, not a
    // coroutine that outlives a dismissed/re-opened dialog), so two overlapping calls for the same
    // logical transaction previously created two independent sets of JournalItemEntity rows (each
    // getting a fresh itemId, so OnConflictStrategy.REPLACE on the voucher header never caught it)
    // and applied applyLedgerDelta twice - a real doubled ledger-balance impact with only one
    // voucher visible in report screens (the header row's own REPLACE hid the first attempt's
    // voucherNumber). This mutex serializes calls to [postVoucher] so a second, overlapping call
    // waits for the first to fully commit (or fail) instead of racing it - closes the concurrent/
    // overlapping-retry case for every caller uniformly. It does NOT create false-idempotency for a
    // genuinely sequential retry after the first call already finished (e.g. minutes later) - for
    // manual one-shot entry with no persisted draft, that case is structurally indistinguishable
    // from a legitimate second real transaction without inventing new persisted state, which is out
    // of scope here (see docs/CORRECTIONS_LOG.md's Section 2 entry). The draft-backed path
    // (postVoucherDraft -> VoucherManagementServiceImpl.postDraft) gets full coverage separately via
    // a stable, draft-id-derived idempotencyKey, which also closes its sequential-retry case.
    private val postVoucherMutex = kotlinx.coroutines.sync.Mutex()
    ...
    suspend fun postVoucher(
    ...
    ): AccountingResult<Voucher> = postVoucherMutex.withLock {
        ...
        if (dbTransaction == null) {
            return@withLock AccountingResult.Failure(AppError.SystemError("Database transaction unavailable: cannot post voucher atomically."))
        }
    ...
    private suspend fun findFiledOrProcessingGstReturnCoveringDate(companyId: String, voucherDateStr: String): GstReturn? {
        ...
        for (entity in candidates) {
            ...
            val range = runCatching { GstPeriod.of(fy, quarter, entity.month, isAnnual = entity.returnType == GstReturnType.GSTR9).dateRange() }.getOrNull() ?: continue
    ...
    suspend fun getOrCreateGstReturn(
        ...
        // Phase 8 - GSTR-9 is always annual; periodKey must be the FY code alone (never a bogus
        // quarter/month key) so re-opening the same year's GSTR-9 finds the same row, not a new one.
        val period = GstPeriod.of(fy, quarter, month, isAnnual = returnType == GstReturnType.GSTR9)
    ...
    /**
     * Phase 8 - builds the real GSTR-3B statutory tables via [Gstr3bReturnBuilder]. Unlike
     * [buildGstr1Data] (outward-only), this reads the period's full, unfiltered-by-direction
     * transaction set - the builder itself splits OUTPUT/INPUT.
     */
    /**
     * Phase 8 - builds the real GSTR-9 annual tables by re-invoking [buildGstr1Data]/
     * [buildGstr3bData] THEMSELVES over the whole financial year's [transactions] (the caller
     * already resolves this via an [isAnnual][GstPeriod.isAnnual] [period]) and handing their
     * output to [Gstr9ReturnBuilder] to regroup - never a third GST calculation. Table 18's inward
     * HSN aggregation is the one genuinely new grouping, computed directly here (mirrors
     * [Gstr1ReturnBuilder]'s own Table 12 HSN grouping exactly, just for INPUT-direction rows).
     */
    private suspend fun buildGstr9Data(companyId: String, entity: GstReturnEntity, period: GstPeriod, transactions: List<GstTransaction>): Gstr9ReturnData {
        val company = dao.getCompanyById(companyId)
        val gstr1 = buildGstr1Data(companyId, entity, period, transactions)
        val gstr3b = buildGstr3bData(companyId, period, transactions)
        val inwardHsn = transactions.filter { it.direction == GstDirection.INPUT }
        return Gstr9ReturnBuilder.build(
            companyGstin = company?.gstin ?: "", fyCode = period.fyCode,
            gstr1 = gstr1, gstr3b = gstr3b, inwardTransactionsForYear = inwardHsn
        )
    }

    private suspend fun buildGstr3bData(companyId: String, period: GstPeriod, transactions: List<GstTransaction>): Gstr3bReturnData {
        val company = dao.getCompanyById(companyId)
        return Gstr3bReturnBuilder.build(companyGstin = company?.gstin ?: "", periodKey = period.periodKey, transactions = transactions)
    }
    ...
    private suspend fun buildGstReturnSections(companyId: String, entity: GstReturnEntity, period: GstPeriod, transactions: List<GstTransaction>): Map<String, Map<String, Any?>> {
        return when (entity.returnType) {
            ...
            // Phase 8 supersedes the original bucket-only pass for GSTR3B - the real, statutorily-
            // numbered Table 3.1/3.2/4/5 tables via Gstr3bReturnBuilder, same "regroup, never
            // recompute" discipline as GSTR-1's own Phase 8A, Part 1 upgrade.
            GstReturnType.GSTR3B -> {
                val data = buildGstr3bData(companyId, period, transactions)
                linkedMapOf(
                    "OUTWARD_3_1" to data.outward.toTree(),
                    "INTER_STATE_UNREG_3_2" to linkedMapOf<String, Any?>("count" to data.interStateUnregistered.size, "rows" to data.interStateUnregistered.map { it.toTree() }),
                    "ITC_4" to data.itc.toTree(),
                    "EXEMPT_INWARD_5" to data.exemptInward.toTree(),
                    "ESTIMATED_CASH_LIABILITY_6_1" to data.estimatedCashLiability.let { linkedMapOf<String, Any?>("igstPaise" to it.igst.paise, "cgstPaise" to it.cgst.paise, "sgstPaise" to it.sgst.paise, "cessPaise" to it.cess.paise) }
                )
            }
            ...
            // Phase 8 - GSTR-9 promoted from visibility-only to a real, built return (see
            // Gstr9ReturnBuilder/GstReturnApplicability.availableReturns). GSTR-9C remains
            // visibility-only - a reconciliation STATEMENT (books vs. an auditor's figures) is a
            // fundamentally different, human-authored document this codebase has no source data
            // for at all (not a regroup of existing facts like every other return here) - kept
            // honest rather than fabricated.
            GstReturnType.GSTR9 -> {
                val data = buildGstr9Data(companyId, entity, period, transactions)
                linkedMapOf(
                    "OUTWARD_4" to data.outward.toTree(),
                    "EXEMPT_OUTWARD_5" to data.exemptOutward.toTree(),
                    "ITC_6" to data.itc.toTree(),
                    "TAX_PAID_9" to linkedMapOf<String, Any?>("rows" to data.taxPaid.map { it.toTree() }),
                    "HSN_OUTWARD_17" to linkedMapOf<String, Any?>("count" to data.hsnOutward.size, "rows" to data.hsnOutward.map { it.toTree() }),
                    "HSN_INWARD_18" to linkedMapOf<String, Any?>("count" to data.hsnInward.size, "rows" to data.hsnInward.map { it.toTree() })
                )
            }
            GstReturnType.GSTR9C -> throw IllegalStateException(
    ...
    suspend fun prepareGstReturn(companyId: String, gstReturnId: String, fy: FinancialYear): AccountingResult<GstReturn> {
        ...
        val period = GstPeriod.of(fy, GstQuarter.valueOf(entity.quarter), entity.month, isAnnual = entity.returnType == GstReturnType.GSTR9)
    ...
    suspend fun validateGstReturn(companyId: String, gstReturnId: String, fy: FinancialYear): AccountingResult<GstReturn> {
        ...
        if (sections.isEmpty() || sections.any { it.status == GstReturnSectionStatus.PENDING }) {
            ...
            if (entity.returnType == GstReturnType.GSTR1) {
                ...
                // Phase 8 - GSTR-3B-specific validation (company's own filing-GSTIN checksum, POS,
                // tax cross-check across BOTH directions) layered on top of the generic
                // Place-of-Supply check above, mirroring the GSTR-1 branch's own structure exactly.
                val company = dao.getCompanyById(companyId)
                val issues = Gstr3bValidator.validate(company?.gstin ?: "", transactions, entity.isNilReturn)
                issues.forEach { issue ->
                    val text = "[${issue.code}] ${issue.message}"
                    if (issue.severity == Gstr1ValidationSeverity.ERROR) errors += text else warnings += text
                    structuredIssues += mapOf(
                        "code" to issue.code, "message" to issue.message,
                        "severity" to issue.severity.name, "voucherId" to null
                    )
                }
            } else if (entity.returnType == GstReturnType.GSTR9) {
                // Phase 8 - GSTR-9-specific validation: company GSTIN, and whether the underlying
                // monthly/quarterly GSTR-1/3B periods this annual return aggregates are themselves
                // actually FILED yet (a real, GSTR-9-specific concern no other return type has).
                val company = dao.getCompanyById(companyId)
                val yearReturns = dao.getGstReturnsForCompany(companyId).first().filter {
                    it.financialYearId == entity.financialYearId && (it.returnType == GstReturnType.GSTR1 || it.returnType == GstReturnType.GSTR3B)
                }
                val filedCount = yearReturns.count { it.status == GstReturnStatus.FILED }
                val issues = Gstr9Validator.validate(
                    company?.gstin ?: "", filedCount, yearReturns.size, entity.isNilReturn, transactions.isNotEmpty()
                )
                issues.forEach { issue ->
                    val text = "[${issue.code}] ${issue.message}"
                    if (issue.severity == Gstr1ValidationSeverity.ERROR) errors += text else warnings += text
    ...
    suspend fun exportGstReturnAs(companyId: String, gstReturnId: String, fy: FinancialYear, format: ExportFormat): AccountingResult<ExportResult> {
        ...
        val entity = dao.getGstReturnById(companyId, gstReturnId)
            ?: return AccountingResult.Failure(AppError.ResourceNotFound("GstReturn", gstReturnId))
        if (entity.returnType != GstReturnType.GSTR1 && entity.returnType != GstReturnType.GSTR3B && entity.returnType != GstReturnType.GSTR9) {
            return AccountingResult.Failure(AppError.BusinessRuleViolation("Export is only available for a GSTR1, GSTR3B, or GSTR9 return - this return is ${entity.returnType}."))
        }
        val period = GstPeriod.of(fy, GstQuarter.valueOf(entity.quarter), entity.month, isAnnual = entity.returnType == GstReturnType.GSTR9)
        val transactions = getActiveGstTransactionsForPeriod(companyId, entity.financialYearId, period.dateRange())
        val metadata = buildMetadata(companyId, ExportType.GST_RETURN, entity.financialYearId)
        val content = when (entity.returnType) {
            GstReturnType.GSTR1 -> {
                val data = buildGstr1Data(companyId, entity, period, transactions)
                when (format) {
            ...
            GstReturnType.GSTR9 -> {
                ...
                when (format) {
                    ExportFormat.JSON -> ExportJsonSerializer.serialize(metadata, data.toTree())
                    ExportFormat.CSV -> CsvEngine.write(data.toCsvHeaders(), data.toCsvRows())
                    ExportFormat.GSTR_JSON -> gstReturnJsonAdapter.toJson(ExportJsonSerializer.envelope(metadata, Gstr9PortalJsonSerializer.serialize(data)))
                }
            }
            else -> {
                val data = buildGstr3bData(companyId, period, transactions)
                when (format) {
                    ExportFormat.JSON -> ExportJsonSerializer.serialize(metadata, data.toTree())
                    ExportFormat.CSV -> CsvEngine.write(data.toCsvHeaders(), data.toCsvRows())
                    ExportFormat.GSTR_JSON -> gstReturnJsonAdapter.toJson(ExportJsonSerializer.envelope(metadata, Gstr3bPortalJsonSerializer.serialize(data)))
                }
            }
        }
        return AccountingResult.Success(ExportResult(metadata, format, content))
    ...
    suspend fun importGstReturnDraftJson(companyId: String, gstReturnId: String, jsonContent: String): AccountingResult<GstReturn> {
        ...
        if (parsed == null || parsed["data"] !is Map<*, *>) {
            return AccountingResult.Failure(AppError.ValidationError("The imported file is not a valid ${entity.returnType} draft export (missing 'data')."))
        }
        @Suppress("UNCHECKED_CAST")
        val dataTree = parsed["data"] as Map<String, Any?>
        val now = System.currentTimeMillis()
        val existingSections = dao.getSectionsForGstReturn(gstReturnId).associateBy { it.sectionKey }
        // Phase 8 - GSTR3B's own tree-key -> sectionKey mapping, same shape [Gstr3bReturnData.toTree]
        // produces, mirroring the GSTR1 mapping immediately below exactly.
        val sectionKeys = when (entity.returnType) {
            GstReturnType.GSTR3B -> listOf("outward", "interStateUnregistered", "itc", "exemptInward")
                .zip(listOf("OUTWARD_3_1", "INTER_STATE_UNREG_3_2", "ITC_4", "EXEMPT_INWARD_5"))
            GstReturnType.GSTR9 -> listOf("outward", "exemptOutward", "itc", "taxPaid", "hsnOutward", "hsnInward")
                .zip(listOf("OUTWARD_4", "EXEMPT_OUTWARD_5", "ITC_6", "TAX_PAID_9", "HSN_OUTWARD_17", "HSN_INWARD_18"))
            else -> listOf("b2b", "b2cl", "b2cs", "cdnr", "cdnur", "exports", "nilRated", "hsn", "documentsIssued")
